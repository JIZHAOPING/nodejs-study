let huffman = (function(){
    ...
    function initData(data) {
        ...
        // for(let i=0; i<data.length;i++) data[i]=str.charCodeAt(i);
    }
    /**
      * 统计信源文件中每个符号出现的频次
      *
    ...
    /**
     * 返回字符串中第一次出现index的子字符串
     * 失败返回-1
     * @param {频次占位字符串} strFrq 
     * @param {子字符串} index 
     */
    ...
    function compress(data,output){
        console.log("压缩文件");
        $output = output;
        initData(data);             //初始化文件内容
        statFreq();                 //统计频次
        infoSrcAnalyze();           //分析---计算概率
