...
// var huffman = (function(){
    ...
    Huffman.prototype.GenHfmCode = function GenHfmCode(){
        ...
        for(i = 0;i<this.SNUM_MAX;i++){
            if(this.hfmTree[i].p==0) continue;

            node = this.hfmTree[i];
            pos = i;

            //生成码字
            while((pos==node.p)&&node==this.hfmTree[pos]){
                (pCode++) = (this.hfmTree[node.p].l == pos) ? 1:0;
                pos = (bit == 0x00) ? node.r : node.l;	
                node = this.hfmTree[pos];
            }
            pHfmCode = this.hfmCode[i];
		    while(pCode > code)
		    {
			    (pHfmCode++) = (--pCode);
            }
    ...
    Huffman.prototype.PrintHfmCode = function PrintHfmCode()
    {
	    let i, num = 0;
	    let avgLen = 0;

	    this.printf("码字：\n");
	    this.printf("xi\tpos\tfreq\tlen\tCode\n");
	    this.printf("---------------------------------------------\n");
	    for(i=0; i<this.SNUM_MAX; i++)
	    {
		    if(this.freq[i] != 0)
		    {
			    num++;
			    avgLen += p[i] * (this.hfmCode[i].length);
			    printf(`x${num}\t${i}\t${this.freq[i]}\t${(this.hfmCode[i].length)}\t${this.hfmCode[i]}\n`);
		    }
	    }
	    this.printf("---------------------------------------------\n");
	    this.printf("平均码长：\t%.3f\n", avgLen);
	    this.printf("编码效率：\t%.2f %%\n", 100.0 * Entropy() / avgLen);
	    this.printf("理论压缩率：\t%.2f %%\n\n", 100.0 * avgLen / CHAR_BIT);
    }
